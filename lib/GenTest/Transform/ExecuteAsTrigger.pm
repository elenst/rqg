...
sub transform {
  ...
  #          - everything that causes explicit/implicit COMMIT
	return STATUS_WONT_HANDLE if $orig_query =~ m{(?:OUTFILE|INFILE|PROCESSLIST|CREATE\s|ALTER\s|DROP\s|GRANT\s|FLUSH\s|START|BEGIN|COMMIT|ROLLBACK|SHOW\s|REVOKE\s|TRUNCATE\s|USE\s|UNLOCK\s|PREPARE\s|EXECUTE\s)}sio;
