# DATADIR=/data/src/bb-10.1-marko/data DATADIR2=/data/bld/10.1-rel/data perl ./gentest.pl --dsn1="dbi:mysql:user=root:host=127.0.0.1:port=3306:database=test" --dsn2="dbi:mysql:user=root:host=127.0.0.1:port=3307:database=test" --grammar=tablespace.yy --threads=1 --duration=600  --seed=1 --validators=ExecutionTimeComparator
...
#  ; restore_tablespace ALTER TABLE my_table IMPORT TABLESPACE
    ...
    /*executor2 { $executors->[1]->import_tablespace($my_table, "$ENV{DATADIR}/test/$my_table.ibd.empty", "$ENV{DATADIR}/test/$my_table.ibd"); '' } */
  | 
    /*executor1 { $executors->[0]->import_tablespace($my_table, "$ENV{DATADIR}/test/$my_table.ibd.backup", "$ENV{DATADIR}/test/$my_table.ibd"); '' } */
    /*executor2 { $executors->[1]->import_tablespace($my_table, "$ENV{DATADIR}/test/$my_table.ibd.backup", "$ENV{DATADIR}/test/$my_table.ibd"); '' } */
;
